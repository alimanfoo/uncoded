...
class TestRepoDogfooding:
    ...
    uncoded's own LSP setup is generated by ``setup-serena``. These tests
    assert the invariants the templates are supposed to produce, so a
    change to the templates that isn't mirrored into this repo's files
    (e.g. bumping ``SERENA_VERSION`` without regenerating ``.mcp.json``)
