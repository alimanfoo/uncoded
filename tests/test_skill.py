...
class TestSyncSkills:
    ...
    def test_writes_skill_files(self, tmp_path):
        ...
        for root in SKILL_ROOTS:
            assert (tmp_path / root / "coherence-review" / "SKILL.md").exists()

    def test_creates_parent_directories(self, tmp_path):
        sync_skills(source=True, docs=False, project_root=tmp_path, check=False)
        for root in SKILL_ROOTS:
            assert (tmp_path / root / "coherence-review").is_dir()
    ...
    def test_idempotent(self, tmp_path):
        sync_skills(source=True, docs=False, project_root=tmp_path, check=False)
        mtimes = [
            (tmp_path / root / "coherence-review" / "SKILL.md").stat().st_mtime_ns
            for root in SKILL_ROOTS
        ]
        sync_skills(source=True, docs=False, project_root=tmp_path, check=False)
        assert [
    ...
    def test_check_mode_does_not_write(self, tmp_path):
        ...
        for root in SKILL_ROOTS:
            assert not (tmp_path / root / "coherence-review" / "SKILL.md").exists()
    ...
    def test_removes_legacy_skill_files(self, tmp_path):
        legacy_paths = [
            tmp_path / root / "uncoded-review" / "SKILL.md" for root in SKILL_ROOTS
        ]
...
class TestSyncSkillsProjectRootAnchor:
    def test_project_root_anchors_writes_independent_of_cwd(
        ...
        for root in SKILL_ROOTS:
            assert (tmp_path / root / "coherence-review" / "SKILL.md").exists()
            assert not (sub / root / "coherence-review" / "SKILL.md").exists()
    ...
    def test_project_root_anchors_legacy_removal_independent_of_cwd(
        ...
        for root in SKILL_ROOTS:
            legacy_path = tmp_path / root / "uncoded-review" / "SKILL.md"
        ...
        for root in SKILL_ROOTS:
            assert not (tmp_path / root / "uncoded-review" / "SKILL.md").exists()
