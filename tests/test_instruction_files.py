...
class TestSyncInstructionFile:
    ...
    def test_returns_true_on_first_write(self, tmp_path):
        path = tmp_path / "CLAUDE.md"
        assert sync_instruction_file(path, project_root=tmp_path) is True

    def test_returns_false_when_clean(self, tmp_path):
        path = tmp_path / "CLAUDE.md"
        sync_instruction_file(path, project_root=tmp_path)
        assert sync_instruction_file(path, project_root=tmp_path) is False


class TestSyncInstructionFileCheckMode:
    def test_does_not_create_file(self, tmp_path):
        path = tmp_path / "CLAUDE.md"
        changed = sync_instruction_file(path, project_root=tmp_path, check=True)
